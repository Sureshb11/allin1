// icons.jsx — custom single-colour sport glyphs (24×24, currentColor).
// One coherent family: 1.7px rounded strokes + occasional solid accent.
// <SportIcon id="cricket" size={34} /> renders centred in the cell.

function SportIcon({ id, size = 34, style }) {
  const P = {
  ...
  switch (id) {
    ...
    case 'tennis': // racquet + ball
      body = (<g>
        <ellipse cx="10.2" cy="9.4" rx="5" ry="5.8" {...P} transform="rotate(-32 10.2 9.4)" />
    ...
    case 'tabletennis': // paddle + ball
      body = (<g>
        <ellipse cx="10.5" cy="9.5" rx="5.4" ry="6" {...solid} transform="rotate(-30 10.5 9.5)" />
    ...
    case 'squash': // round-head racquet (distinct from tennis oval) + small ball
      body = (<g>
    ...
    case 'pickleball': // solid paddle + wiffle ball (diamond hole pattern)
      body = (<g>
        <rect x="5.4" y="4.4" width="8.2" height="9.4" rx="3.4" {...solid} transform="rotate(-18 9.5 9)" />
        <path {...P} d="M11 13.6 L13.8 18" />
        <circle cx="17" cy="8.4" r="2.9" {...P} />
        <g fill="currentColor" stroke="none">
          <circle cx="17" cy="6.7" r="0.5" /><circle cx="15.4" cy="8.4" r="0.5" /><circle cx="18.6" cy="8.4" r="0.5" /><circle cx="17" cy="10.1" r="0.5" />
        </g>
      </g>); break;
    case 'judo': // tied martial-arts belt (band + knot + hanging ends)
      body = (<g>
    ...
    case 'snowboard': // board + bindings
      body = (<g transform="rotate(35 12 12)">
    ...
    case 'rummy': // two overlapping playing cards + spade pip
      body = (<g>
        <rect x="3.4" y="7.2" width="9" height="12.4" rx="1.6" {...P} transform="rotate(-14 7.9 13.4)" />
...
Object.assign(window, { SportIcon });
